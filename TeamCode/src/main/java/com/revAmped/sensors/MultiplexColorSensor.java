...
public class MultiplexColorSensor {
    ...
    private volatile COLOR_SENSOR currentColorSensor = null;
    ...
    public synchronized int[] getCRGB(COLOR_SENSOR sensor) {
        // don't change sensor if it is already the current sensor
        if (currentColorSensor != sensor) {
            // Write to I2C port on the multiplexer
            muxReader.write8(0x0, 1 << sensor.channel, WRITTEN);
            currentColorSensor = sensor;
        }
  ...
  /* Calculate the final CCT */
    ...
    public static Boolean isRed(int[] crgbF,
        ...
        RobotLog.i("Color isRed: " + isRed + " Front hue: " + hsvF[0] + " lux: " + luxF + " Back hue: " + hsvB[0] + " lux: " + luxB);
        return isRed;
    }

    /**
     * Use INT to control LED.
     * wire LED to INT pin
     * @param enableLED
     */
    void setInterrupt(boolean enableLED) {
        byte r = adaReader.read8(ENABLE);
        if (enableLED) {
            r |= AIEN;
        } else {
            r &= ~AIEN;
        }
        adaReader.write8(ENABLE, r, WRITTEN);
    }
}
