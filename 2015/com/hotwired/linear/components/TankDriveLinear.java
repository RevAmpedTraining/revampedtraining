    ...
    extends TankDrive
{
                            ...
                            HwGyro gyroSensor)
    {
        ...
        this.motors = new HwMotorsLinear(driveLeftFront,
                                         driveLeftBack,
                                         driveRightFront,
                                         driveRightBack);
    }
        ...
        throws InterruptedException
    {
        ...
        if (timeoutMillis < 0) {
            timeoutMillis = Math.abs((int) (tick * Robot.AM40_MILLISECOND_PER_TICK * MOVE_TIMEOUT_FACTOR)) + 2000;
        }
        ...
        while (op.opModeIsActive() &&
            ...
            if (!isAccelerate)
            {
                basePower = HwMath.sign(delta, maxPower);
            }
            else
            {
                if (Math.abs(average) < Math.abs(tick / 3))
                {
                    ...
                    basePower = (average/1600f) + HwMath.sign(tick, POWER_START);
                }
                else
                {
                    // ramp down for 2/3 of distance
                    // minimum power is START_POWER
                    basePower = (delta/3200f) + HwMath.sign(delta, POWER_STOP);
                }
            ...
            if (stalled.isStalled(average, timeStamp)) {
                ...
                setPower(Robot.AM40_ENCODER_RATIO * basePower);
            }
        ...
        throws InterruptedException
    {
        ...
        while (op.opModeIsActive() &&
            ...
            if (!isAccelerate)
            {
                power = HwMath.sign(delta, maxPower);
            }
            else {
                if (Math.abs(heading) < Math.abs(deg / 3f)) {
                    // ramp up
                    power = HwMath.sign(deg, Kp_up * Math.abs(heading) + POWER_START);
                    //power = HwMath.sign(deg, maxPower);
                } else {
                    // ramp down
                    power = Kp_down * delta + HwMath.sign(delta, POWER_STOP);
                }
            }

            power = Range.clip(power, -maxPower, maxPower);
            setTurn(Robot.AM40_ENCODER_RATIO * power,
                    turnType);
